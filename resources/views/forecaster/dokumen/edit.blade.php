    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('forecaster.dokumen.update', $dokumen->id) }}" method="POST"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul"
                                name="judul" value="{{ old('judul', $dokumen->judul) }}" maxlength="150" required>
                            @error('judul')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="file" class="form-control @error('file') is-invalid @enderror" id="file"
                                name="file" accept=".pdf,.doc,.docx" @if (!$dokumen->file_path) required @endif>
                            @error('file')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <ul class="list-unstyled mb-0 small">
                                            <li><strong>Tanggal Upload:</strong>
                                                {{ $dokumen->tanggal_upload->format('d M Y') }}</li>
                                            <li><strong>Diupload Oleh:</strong> {{ $dokumen->forecaster->nama }}</li>
                                            <li><strong>Terakhir Diupdate:</strong>
                                                {{ $dokumen->updated_at->format('d M Y H:i') }}</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="card bg-light">
                                    <div class="card-body">
                                        ...
                                        <ul class="list-unstyled mb-0 small">
                                            <li><i class="fas fa-check text-success me-2"></i>Perubahan akan langsung
                                                terlihat</li>
                                            <li><i class="fas fa-check text-success me-2"></i>Maskapai akan melihat versi
                                                terbaru</li>
                                            <li><i class="fas fa-check text-success me-2"></i>File lama akan dihapus jika
                                                diganti</li>
                                        </ul>
                                    </div>
                                </div>
                    ...
                    <!-- Hidden Delete Form -->
                    <form id="deleteForm" action="{{ route('forecaster.dokumen.delete', $dokumen->id) }}" method="POST"
                        style="display: none;">
                        @csrf
                        @method('DELETE')
                    </form>
...
@push('scripts')
    <script>
        ...
        function confirmDelete() {
            if (confirm(
                    'Yakin ingin menghapus dokumen ini? Tindakan ini tidak dapat dibatalkan dan akan menghapus file dari server.'
                    )) {
